...
def dashboard_dos_view(request):
    context = dict(
       # Agrega aquí cualquier contexto adicional que necesites
       admin.site.each_context(request),
    )
    return TemplateResponse(request, "admin/index2.html", context)

# Agrega la URL de la nueva vista al panel de administración
class CustomAdminSite(admin.AdminSite):
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('dashboard-dos/', self.admin_view(dashboard_dos_view), name="dashboard_dos"),
        ]
